...
use super::super::resource::Config;
...
pub fn set_player_movements(
    ...
    match query.get_single_mut() {
        Ok((p, mut velocity)) => {
            ...
            velocity.x.min(-config.player.move_speed);
            velocity.x.max(config.player.move_speed);
            velocity.y.min(-config.player.move_speed);
            velocity.y.max(config.player.move_speed);
        },
